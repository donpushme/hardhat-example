...
/**
 * @title TeamBVault
 * @dev Vault contract for holding bets for Team B across all events
 */
contract TeamBVault is Ownable, ReentrancyGuard {
    // Reference to the main Klaus Bet contract
    address public klausBet;

    // Reference to the complementary vault (Team A)
    address public teamAVault;
    ...
    /**
     * @dev Set the complementary vault address (Team A)
     * @param _teamAVault Address of the Team A vault
     */
    function setTeamAVault(address _teamAVault) external onlyOwner {
        require(_teamAVault != address(0), "Invalid vault address");
    ...
    /**
     * @dev Place a bet on Team B for an event
     * @param _eventId ID of the event
    ...
    function placeBet(uint256 _eventId, uint256 _amount) external nonReentrant {
        ...
        // Try to match bets with Team A vault
        _tryMatchBets(_eventId);
    }

    /**
     * @dev Try to match bets with the Team A vault
     * @param _eventId ID of the event
     */
    function _tryMatchBets(uint256 _eventId) internal {
        ...
        uint256 oddsB = eventOdds[_eventId];

        // Calculate how much from Team A is needed to match Team B bets
        // For Team B bets, required amount from Team A = (Team B amount * oddsB / 100)
        uint256 requiredAmount = (totalAmounts[_eventId] * oddsB) / 100;

        // Check Team A vault's matched amount for this event
        uint256 compMatchedAmount = TeamAVault(teamAVault)
            .getTotalMatchedAmount(_eventId);

        // If there's enough in Team A vault, match our bets
        if (compMatchedAmount >= requiredAmount) {
            ...
            // Inform Team A vault to update its matching
            TeamAVault(teamAVault).updateMatching(_eventId, newlyMatchedAmount);
        }
    ...
    /**
     * @dev Team A vault calls this to update matching after it receives new bets
     * @param _eventId ID of the event
     * @param _newlyMatchedAmount Amount newly matched in Team A vault
     */
    function updateMatching(
        uint256 _eventId,
        uint256 _newlyMatchedAmount
    ) external {
        require(msg.sender == teamAVault, "Only Team A vault can call");
    ...
    /**
     * @dev Distribute winnings when Team B wins
     * @param _eventId ID of the event
    ...
    function distributeWinnings(
        ...
        // Get losing bets from Team A vault
        uint256 totalWinnings = TeamAVault(teamAVault).getEventBalance(
            _eventId
        );
        ...
        // Transfer all tokens from Team A vault for this event
        TeamAVault(teamAVault).transferEventFunds(_eventId, address(this));
    ...
    function transferEventFunds(
        ...
        require(
            msg.sender == teamAVault || msg.sender == klausBet,
            "Unauthorized"
        );
